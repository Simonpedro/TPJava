...
/**
 * Servlet implementation class GuardarElectrodomesticoServlet
 */
@WebServlet("/GuardarElectrodomesticoServlet")
public class GuardarElectrodomesticoServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GuardarElectrodomesticoServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			new ElectrodomesticoLogic().save(elec);
			request.setAttribute("msj", "Electrodomestico guardado con exito");
			RequestDispatcher rd = request
					.getRequestDispatcher("/index.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
			request.setAttribute("msj", "No se pudo realizar el alta");
			RequestDispatcher rd = request
					.getRequestDispatcher("/index.jsp");
			rd.forward(request, response);
		}
